...
public class Dialogc extends JFrame{
	...
	//Creates new window based on the config file
	public Dialogc(){
		super("Dialogc");
		setSize(1000,600);
		setResizable(false);
	...
	public void compile(){
		...
		if(currentFile!=null){
			...
			if(PM_parse(object, currentFile.getName(), '#')!=0){
				...
				try {
					writer = new PrintWriter(directory +"/Compiled.java", "UTF-8");
					...
					writer.println("public class Compiled "+"implements "+PM_getString(object, "title")+"FieldEdit"+"{");
					...
					writer.println("    public Compiled(){");
					...
					for(i=0; i<buttonsArray.length; i++){
						...
						writer.println(format);
						/*writer.println("        button.addActionListner(new ActionListener(){");
					...
					writer.println("        Compiled window = new Compiled();");
					writer.println("    }");
					writer.println("}");
					writer.close();
					writer = new PrintWriter(directory+"/"+PM_getString(object, "title")+"FieldEdit.java","UTF-8");
					writer.println("import java.awt.*;");
					writer.println("import javax.swing.*;");
					writer.println("import java.awt.event.ActionListener;");
					writer.println("import java.awt.event.ActionEvent;");
					writer.println("public interface "+PM_getString(object, "title")+"FieldEdit"+"{");
					for(i=0; i<fieldsArray.length; i++){
						writer.println("    public String getDC"+fieldsArray[i]+"();");
						writer.println("    public void setDC"+fieldsArray[i]+"(String input);");
					}
	...
	public void run(){
		try {
			Runtime.getRuntime().exec("javac -d "+directory+" Compiled.java");
			Runtime.getRuntime().exec("javac -d "+directory+" "+PM_getString(object, "title")+"FieldEdit.java");
		} catch (IOException e) {
			
		}
		try{
			Runtime.getRuntime().exec("java"+" -cp "+directory+" Compiled");
		}catch(Exception e){
